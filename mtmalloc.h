...
class MemList {
 public:
  ...
  void pushNode(MemBlock node) {
    pushList(node, node, 1);
  }

  // Pop one linked nodes at front
  MemBlock popNode() {
    return popList(1).first;
  }
...
class Span {
 public:
  Span(size_t firstPageID, size_t firstPageOffset, size_t totalPageNum)
    : firstPageID_{firstPageID}, firstPageOffset_{firstPageOffset}, totalPageNum_{totalPageNum} {}
  ...
  size_t getFirstPageID() {
    return firstPageID_;
  }

  size_t getFirstPageOffset() {
    return firstPageOffset_;
  }

  size_t getTotalPageNum() {
    return totalPageNum_;
  }

  // Get the begin address of paged memory
  char* getBeginAddr() {
    return reinterpret_cast<char*>(firstPageID_ * System::kPageSize + firstPageOffset_);
  }
...
class RadixTreeV3 : public Singleton<RadixTreeV3> {
 ...
 public:
  ...
  Span* search(size_t pageID) {
    ...
    if (root_->child_[i1] == nullptr || root_->child_[i1]->child_[i2] == nullptr) [[unlikely]] {
      return nullptr;
    }
...
class SizeHandler {
 public:
  ...
  size_t getCacheIndex() {
    ...
    for (const auto& config : cacheIndexConfigs_) {
      if (size_ > config.minSize) {
        ...
        cacheIndex_ = static_cast<int>(config.startIndex + ((delta + config.alignNum - 1) / config.alignNum - 1));
        return cacheIndex_;
      }
    }
    std::abort();
  ...
  size_t getHeapIndex() {
    ...
    heapIndex_ = static_cast<int>((getBatchNum() * size_) / System::kPageSize);
...
class PageHeap : public Singleton<PageHeap> {
 ...
 public:
  ...
  Span* acquire(SizeHandler& sizeHandler) {
    ...
    for (size_t i = bucketIndex + 1; i < kMaxBuckets; i++) {
      if (!spanBuckets_[i].empty()) {
        ...
        Span* retSpan = ObjectPool<Span>::getInstance().acquire(
          firstFitSpan->getFirstPageID(), firstFitSpan->getFirstPageOffset(), pagesToAcquire);
        recordPopSpan(retSpan);

        firstFitSpan->shrinkLeftBound(pagesToAcquire);
        spanBuckets_[firstFitSpan->getTotalPageNum()].push_front(firstFitSpan);
    ...
    Span* maxSpan = ObjectPool<Span>::getInstance().acquire(getPageID(addr), getPageOffset(addr), pagesToAlloc);
  ...
  void release(Span* span) {
    ...
    while (true) {
      ...
      if (prevSpan->getTotalPageNum() + span->getTotalPageNum() >= kMaxBuckets) {
        break;
      }
    ...
    while (true) {
      size_t nextPageID = span->getFirstPageID() + span->getTotalPageNum();
      ...
      if (nextSpan->getTotalPageNum() + span->getTotalPageNum() >= kMaxBuckets) {
        break;
      }
 ...
 private:
  ...
  static size_t getPageOffset(void* addr) {
    return reinterpret_cast<size_t>(addr) - (getPageID(addr) * System::kPageSize);
  }

  static Span* OutOfIndexAcquire(const size_t bucketIndex) {
    void* addr = System::allocate(bucketIndex * System::kPageSize);
    Span* res = ObjectPool<Span>::getInstance().acquire(getPageID(addr), getPageOffset(addr), bucketIndex);
...
class CentralCache : public Singleton<CentralCache> {
 ...
 private:
  ...
  // Each bucket’s lock must be acquired ​​before accessing the bucket and its span
...
class ThreadCache {
 public:
  ...
  void release(MemBlock memblock, SizeHandler sizeHandler) {
    ...
    // If bucket length > threshold, release a list of memblock to CentralCache
 ...
 private:
  ...
  MemBlock fetchFromCentralCache(SizeHandler& sizeHandler) {
    ...
    auto [head, tail, n] = CentralCache::getInstance().acquire(sizeHandler, fixedBatchNum);
    if (n > 1) {
