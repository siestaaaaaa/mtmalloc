...
inline constexpr size_t TCThreshold = 256 * 1024;
...
class Helper {
 public:
  ...
  static size_t bytesToIndex(size_t bytes) {
    assert(bytes > 0 && bytes <= TCThreshold);
...
class ThreadCache {
 public:
  ...
  void deallocate(void* ptr, size_t size) {
    assert(ptr != nullptr);
    assert(size > 0 && size <= TCThreshold);
